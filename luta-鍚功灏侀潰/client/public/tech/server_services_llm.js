...
import { DEGREES, ANALYZE_SYSTEM, DEGREE_SELECT_SYSTEM, PROMPT_SYSTEM, HARD_NEGATIVES, STYLE_DNA, BACKGROUND_COLORS, PRIMARY_COLOR_EXAMPLES, FIVE_COLORS, DEGREE_COLOR_RULES, generateColorScheme } from '../prompts/system.js';
...
function getLLMErrorMessage(err, stage) {
  ...
  return `${stage}失败：${err?.response?.data?.error || err?.message || '未知错误'}`;
}
    ...
    for (let attempt = 1; attempt <= maxAttemptsPerModel; attempt++) {
      try {
        ...
        const response = await axios.post(
          `${config.API_BASE}/v1/chat/completions`,
          {
            model,
            messages,
            temperature,
        ...
        lastErr = err;
        const status = err?.response?.status;
        console.error(`[${stage}] 模型 ${model} 失败 (${status || err.code || err.message})`);
...
export async function analyzeContent(podcastContent) {
  console.log('[analyzeContent] 开始分析，使用模型:', config.TEXT_MODEL);
...
export async function selectDegree(podcastContent) {
  try {
    ...
    return {
      ...
      missingInfo: Array.isArray(result?.missingInfo) ? result.missingInfo.filter(Boolean).slice(0, 3) : []
    };
  } catch (err) {
    throw err;
  }
...
### 对比方式
- 值：${structureParams.contrastMethod || 'none'}
...
**可调细节**（仅限以下范围）：
- 边缘偏好：${bias.edge || 'soft'}
- 材质质感：${bias.material || 'smooth'}
- 节奏微调：${bias.rhythm || 'static'}
- 颜色温度：${atmosphere.temperature || 'neutral'}

**颜色倾向**：
- 策略：${degree.colorTendency || '高明度，清澈轻松'}
- 背景参考：${bgColorRef}
${colorSchemeDesc}

**约束**：
- 形体 ≤ ${degree.constraints?.shapes || 4}
- 线条 ≤ ${degree.constraints?.lines || 3}
- 留白 ≥ ${degree.constraints?.whitespace || 50}%
...
`;
  ...
  console.log('[generatePrompt] 调用 API，使用模型:', config.TEXT_MODEL);
